...
class Queue_adt:
    ...
    def push(self, data):
        if self.front == -1: # if new queue, reset pointers
            self.front = self.rear = 0
        else:
            self.rear = (self.rear + 1) % len(self.queuelist)
    ...
    def pop(self):
        ...
        if self.front == self.rear: # If only one element left in the queue
            ...
            self.front = (self.front + 1) % len(self.queuelist)
    ...
    def count(self):
        if self.front == -1:  #if front is -1, the queue is empty
            ...
            elements = (self.rear - self.front + 1) % len(self.queuelist)
